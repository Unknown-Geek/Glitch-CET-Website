    ...
    return (
        <div className="px-5 mb-[100px] ">
            ...
            <div className={`flex flex-row  mb-[70px]  `}>
                <div className={`flex w-fit ${stopScrolling?"":"Scroll-Left"} `} > 
                    {events.map((event, index) => {
                        ...
                        {/* Previous event card */ }
                        return (
                            <div onClick={() => toggleCard(id)} className={`relative z-10 border-[1px] rounded-[3px] flex flex-col items-center justify-center py-[20px] cursor-pointer mx-[20px] ${clicked ? "w-[450px] lg:w-[800px] " : "w-[320px]  lg:w-[340px] "}  `}>


                                {/* background image */}
                                <img className={`absolute z-0 pointer-events-none opacity-[0.2] brightness-[1.6] ${clicked ? "h-[205%] w-[125%] lg:w-[105%] " : "h-[95%]"} `} src="DotsBG.png" alt="" />

                                {/* title */}
                                <div className="flex flex-row items-center justify-center px-[10px] lg:px-[20px] ">
                                    ...
                                    <span className={`leading-none px-[10px] ${clicked ? "text-[50px] lg:text-[70px] " : "text-[35px]"} `} style={{ fontFamily: "Neopixel" }}>{title}</span>
                                </div>
                                <span className={`text-center my-[20px] font-light ${clicked ? "text-2xl lg:text-3xl " : "text-xl "} `} style={{ fontFamily: "Neopixel" }}>{subTitle} </span>

                                {/* event image and description */}
                                <div className="flex flex-row mt-[30px] lg:mt-[60px] justify-center items-center ">
                                    <img className={`w-[170px] lg:w-[250px] px-[10px] lg:px-0  ${clicked ? "h-[50%] lg:h-[90%] " : ""} `} src='event.png' alt="event" />
                                    {clicked && (
                                        <div className="ml-[5px] lg:ml-[15px] ">
                                            <span style={{ fontFamily: "Pixelify Sans" }} className="flex justify-center items-center w-[250px] lg:w-[370px] pl-[30px] text-[15px] lg:text-xl ">{description1}</span>
                                            <span style={{ fontFamily: "Pixelify Sans" }} className="flex justify-center items-center w-[250px] lg:w-[370px] pl-[30px] text-[15px] lg:text-xl mt-[25px] ">{description2}</span>
                                        </div>
                                    )}
                                </div>

                                <div className={`${clicked ? "flex flex-row mt-[50px] px-[10px] lg:px-[20px] " : "px-[10px] sm:px-[5px] md:px-[10px] lg:px-[20px] "}`}>
                                    {/* lineBar */}
                                    <div className={` mt-[20px] ${clicked ? "flex flex-col" : "flex flex-row"} `}>
                                    ...
                                    {/* Date */}
                                    <div className={`text-center mt-[5px] ${clicked ? "flex flex-row text-center px-[10px] " : ""} `}>
                                        <span style={{ fontFamily: "Morton" }} className={` ${clicked ? "text-4xl lg:text-7xl mt-[20px] lg:mt-[0px] " : "text-6xl lg:text-7xl"} `}>{day}</span>
                                        <div className={` ${clicked ? "flex justify-center items-center ml-[8px] lg:ml-[16px] " : ""} `}>
                                            <span style={{ fontFamily: "Morton" }} className={`text-2xl text-[#ED246D] px-[8px] ${clicked ? "text-3xl lg:text-5xl " : ""} `}>{month}</span>
                                            <span style={{ fontFamily: "Morton" }} className={`text-2xl leading-none ${clicked ? "text-3xl lg:text-5xl ml-[6px] lg:ml-[12px] mr-[10px] " : ""} `}>{clicked ? "2025" : "25"} </span>
                                        </div>
                                    </div>
